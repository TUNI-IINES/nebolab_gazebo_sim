...
class ViconLoc(Node):
    def __init__(self):
        ...
        for robot_index in range(self.robot_count):
            ...
            self.pos_ahead_publishers[tb_name] = self.create_publisher(
                Pose2D, f"/{tb_name}/pos", 10
            )

            self.get_logger().info(f"Creating pos center publisher: /{tb_name}/posc")
            self.pos_center_publishers[tb_name] = self.create_publisher(
                Pose2D, f"/{tb_name}/posc", 10
            )
    ...
    def odom_callback(self, msg, index):
        ...
        self.pose_ahead[index].x = self.pose_center[
            index
        ].x + self.tb_l_si2uni * np.cos(p_angle)
        self.pose_ahead[index].y = self.pose_center[
            index
        ].y + self.tb_l_si2uni * np.sin(p_angle)
