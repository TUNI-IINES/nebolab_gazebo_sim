...
class draw2DUnicyle():
    def __init__(self, ax, init_pos, init_theta=None, field_x=None, field_y=None, pos_trail_nums=0):
        ...
        for i in range(self.__robot_num // len(cl)):
            ...
            # TODO: fix this issue, decide how to deal with plotitng large number of robots
    ...
    def update(self, all_pos, all_theta):
        for i in range(self.__robot_num):
            self.__draw_icon(i, np.array([all_pos[i, 0], all_pos[i, 1], all_theta[i]], dtype=object),
                             arrow_col=self.__colorList[i])
    ...
    def __draw_icon(self, id, robot_state, arrow_col='b'):  # draw mobile robot as an icon
        ...
        if self.__patch_info[id] is None:  # first time drawing
            ...
            self.__patch_info[id][1] = self.__ax.quiver(ar_st[0], ar_st[1], ar_d[0], ar_d[1],
                                                        scale_units='xy', scale=1, color=arrow_col,
